...
async function init() {
  ...
  try {
    ...
    els.hint.textContent = "在这个弹窗里识别歌曲；识别后点击结果即可打开网易云。";

    const response = await sendToContent("GET_STATE");
...
async function ensureContentScript(tabId) {
  await stopLegacyContentScript(tabId);

  try {
    const response = await chrome.tabs.sendMessage(tabId, { target: "partycue-content", type: "PING" });
    if (response?.ok) {
      return;
    }
  } catch {
...
async function stopLegacyContentScript(tabId) {
  try {
    await chrome.tabs.sendMessage(tabId, { type: "STOP_LISTENING" });
  } catch {
    // Older content scripts may not be present in this tab.
  }
}

async function sendToContent(type) {
  const response = await chrome.tabs.sendMessage(activeTab.id, { target: "partycue-content", type });
  if (!response?.ok) {
